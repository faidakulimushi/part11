...
const PokemonPage = ({ previous, next }) => {
  ...
  const type = pokemon.types && pokemon.types.find(type => type.slot === 1)
  ...
  const normalAbility = pokemon.abilities && pokemon.abilities.find(
    (ability) => ability.is_hidden === false
  )

  const hiddenAbility = pokemon.abilities && pokemon.abilities.find(
  ...
  return (
    <>
      ...
      <div
        className={`pokemon-page pokemon-type-${type ? type.type.name : ''}`}
      >
        <div
        ...
        <div className="pokemon-info">
          ...
          <div className="pokemon-abilities">
            {normalAbility && normalAbility.ability && (
              <PokemonAbility
                abilityName={formatName(normalAbility.ability.name)}
              />
            )}

            {hiddenAbility && hiddenAbility.ability && (
              <PokemonAbility
